...
export const AddCat: FC = () => {
    ...
    const handleChange = (fieldName: keyof NewCatFormState) => (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => dispatch({
        fieldName,
    ...
    return (
        <Card style={{width: "50%"}}>
            ...
            <Stack spacing="xs">
                <TextInput label="Name" onChange={handleChange("name")}
                           value={name ?? ''}/>
                <Textarea
                    label="Description"
                    value={description ?? ""}
                ...
                <NumberInput
                    ...
                    onChange={(val) => dispatch({fieldName: "adaptability", value: val.toString()})}
                />
                <NumberInput
                    label="Affection Level"
                    value={affection_level ?? 0}
                    onChange={(val) => dispatch({fieldName: "affection_level", value: val.toString()})}
                />
                <NumberInput
                    label="Child Friendliness"
                    value={child_friendliness ?? 0}
                    onChange={(val) => dispatch({fieldName: "child_friendliness", value: val.toString()})}
                />
                <NumberInput
                    label="Intelligence"
                    value={intelligence ?? 0}
                    onChange={(val) => dispatch({fieldName: "intelligence", value: val.toString()})}
                />
                <NumberInput
                    label="Shedding Level"
                    value={shedding_level ?? 0}
                    onChange={(val) => dispatch({fieldName: "shedding_level", value: val.toString()})}
                />
                <Checkbox
                    label="Hypoallergenic"
                    checked={hypoallergenic ?? false}
                    onChange={(evt) => {
                        dispatch({fieldName: "hypoallergenic", value: evt.currentTarget.value})
                    }}
                />
                <Checkbox
                    label="Indoor"
